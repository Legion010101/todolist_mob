...
const Main = ({navigation, route}: PropsMain) => {
  ...
  const deleteTodo = (key: string, isAddBin = true) => {
    ...
    const deleteTodo = MainTodos.find(item => item.key === key)
  ...
  return <View style={styles.container}>
    <Text
      style={styles.title}>{Boolean(MainTodos.length) ? "Ваш список дел:" : "Список дел пуст... Добавьте дело!"}</Text>
    <View>{MainTodos.map((todo, key) => {
    ...
    <ConfirmDeleteModal modalVisible={modalVisible} setModalVisible={setModalVisible} deleteTodo={deleteTodo} deleteKey={deleteKey}/>
  </View>
}
export default Main
